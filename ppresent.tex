...
\begin{frame}{Splitting Field}
     ...
     Such a minimal field \(F\) which contains all roots of \(f \in K[x]\) is called a splitting field of \(f\) over \(K\) \cite{hunger} because \(f\) splits into linear factors in \(F\).
 \vspace{3mm}
...
\vspace{2mm}
  \begin{tcolorbox}[colback=white, colframe=blue!40, boxsep=1mm]
    \begin{theorem}[Characterization of Galois Groups]
      ...
      \begin{enumerate}
      \item[i)] \(G\) is  a subgroup of symmetric group \(S_n\).
      \item[ii)] If \(f\) is separable, then \(G\) isomorphic to a transitive subgroup of \(S_n\) \cite{hunger}.
      \end{enumerate}
    \end{theorem}
  \end{tcolorbox}
  \vspace{3mm}
\textcolor{green!50!black}{So, galois group of a quartic is a subgroup of \(S_4\).}
\end{frame}
...
\vspace{5mm}
 \begin{tcolorbox}[colback=white, colframe=brown!80!black, boxsep=2mm, title={\bfseries \color{white} Example}]
...
\begin{frame}{Conclusion of my Research}
  \begin{enumerate}
